...
def chat_conversations_list_view(request):
    ...
    if connection_request_id:
        try:
            ...
            return Response(
                {"error": f"Invalid connection request ID: {str(exc)}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not conversation:
            if connection_request.status != "accepted":
                return Response(
                    {"error": "Connection request is not accepted yet."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        if str(conversation.client.id) != str(user.id) and str(
            conversation.lawyer.id
        ) != str(user.id):
            return Response(
    ...
    serializer = ChatConversationSerializer(
        user_conversations, many=True, context={"request": request}
    )
    return Response(serializer.data, status=status.HTTP_200_OK)
...
def chat_messages_view(request, conversation_id):
    ...
    if message_type == "document" and not document_id:
        return Response(
            {"error": "Document ID is required for document messages."},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    try:
        ...
        traceback.print_exc()
        return Response(
            {"error": f"Failed to create message: {str(exc)}"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
