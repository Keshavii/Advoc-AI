...
def get_user_from_token(token):
    ...
    try:
        print(
            f"Backend: Attempting to authenticate token: {token[:30]}..."
        )  # Log token start
        access_token = AccessToken(token)
        user_id = access_token["user_id"]
        user = User.objects.get(id=user_id)
        print(
            f"Backend: Token authenticated successfully for user: {user.username}"
        )  # Log success
        return user
...
class TokenAuthMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        print(
            f"Backend: User in scope after TokenAuthMiddleware: {scope['user']}"
        )  # Log user in scope
