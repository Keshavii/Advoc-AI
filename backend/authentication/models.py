...
class User(Document):
    ...
    role = StringField(
        max_length=32, default="client", choices=("client", "lawyer", "admin")
    )
    is_lawyer_verified = BooleanField(default=False)
    lawyer_verification_status = StringField(
        max_length=32, default="not_submitted"
    ...
    meta = {
        "collection": "users",
        "indexes": ["email", "username", {"fields": ["google_id"], "sparse": True}],
    }
    ...
    def create_user(cls, email, username, password=None, **extra_fields):
        ...
        user = cls(email=email.lower(), username=username, **extra_fields)
        if password:
...
class ChatMessage(Document):
    ...
    )  # Reference to document if shared
