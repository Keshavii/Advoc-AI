...
def _generate_pdf_from_markdown(markdown_content):
    ...
    """
    print(
        f"Markdown content received: {markdown_content[:500]}..."
    )  # Log first 500 chars
...
def upload_signature(request):
    ...
    if not file_obj:
        return Response(
            {"error": 'No file uploaded. Use form field name "signature".'}, status=400
        )

    try:
        upload_result = cloudinary.uploader.upload(file_obj)
        print(
            f"Cloudinary upload result: {upload_result}"
    ...
    if new_status == "accepted":
        ...
        if not existing_chat:
            try:
                ...
                print(
                    f"Created chat conversation {chat_conversation.id} for connection {connection_request.id}"
                )
                ...
                print(
                    f"Created welcome message for conversation {chat_conversation.id}"
                )
            except Exception as e:
...
def download_latest_conversation_pdf(request, pk):
    ...
    if (
        not conversation
        or "document_versions" not in conversation
    ...
    try:
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{conversation.get("title", "legal_document")}.pdf"'
        )
        return response
    except Exception as e:
        return Response(
...
def download_version_pdf(request, pk, version_number):
    ...
    try:
        ...
        if (
            ...
            return Response(
                {"error": "No document versions found for this conversation."},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        version = next(
            (
                v
                for v in conversation["document_versions"]
        ...
        filename = (
            f"{conversation.get("title", "legal_document")}_v{version_number}.pdf"
        )
