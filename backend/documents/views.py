...
from documents.mongo_client import (
...
from .comment_mongo_client import (
...
def create_conversation_with_chat(request):
    ...
    initial_document_content = request.data.get(
        "document_content", ""
    )  # Can be empty for initial creation
    ...
    try:
        ...
        if "```json" in ai_raw_response:
            ...
            ai_response_content = (
                ai_raw_response  # If not JSON, treat raw response as content
            )
        ...
        messages = [
            ...
            },  # Save raw AI response to messages
        ]
...
def send_chat_message(request, pk):
    ...
    """
    message = request.data.get("message")
    document_content = request.data.get(
        "document_content"
    )  # Current document content as context
    ...
    try:
        ...
        updated_messages.append(
            {"sender": "bot", "text": ai_raw_response}
        )  # Save raw AI response to messages
...
def document_comments(request, document_id):
    if request.method == "GET":
        try:
            ...
            return Response(
                {
                    "error": "Failed to load comments. Please check server logs for details."
                },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        ...
        try:
            ...
            new_comment_doc = add_comment(
                document_id, user, content, position, parent_comment_id
            )
            ...
            if new_comment_doc:
                ...
                async_to_sync(channel_layer.group_send)(
                    document_group_name,
                    {"type": "new_comment", "comment": serialized_comment},
                )
...
def generate_share_link(request):
    ...
    if document_id:
        ...
        try:
            ...
            return Response(
                {"error": f"Error generating share link: {e}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    else:
        # Create new document
...
def conversation_list(request):
    ...
    if request.method == "GET":
        ...
        shared_with_users = request.data.get(
            "shared_with_users", []
        )  # New: get shared_with_users

        print(
            f"[DEBUG Backend] conversation_list (POST) - Received messages: {messages}"
        )
...
def conversation_detail(request, pk):
    ...
    if request.method == "GET":
        ...
        if (
            ...
            and conversation.get("owner") == request.user.username
        ):
            user_has_access = True

        # 2. Check public share permissions
        share_permissions = conversation.get("share_permissions")
        if share_permissions and share_permissions.get("permission_level") in [
            "view",
            "edit",
        ]:
            user_has_access = True
        ...
        print(
            f"[DEBUG Backend] conversation_detail (PUT) - Received messages: {messages}"
        )
        ...
        messages_to_update = (
            ...
            else existing_conversation.get("messages", [])
        )
...
def share_document_with_user(request, pk):
    ...
    permission_level = request.data.get(
        "permission_level"
    )  # 'view', 'edit', or None to remove
    ...
    try:
        success = update_user_share_permissions(
            pk, username_to_share_with, permission_level
        )
        if success:
            if permission_level:
                return Response(
                    {
                        "status": f"Document shared with {username_to_share_with} with {permission_level} permissions."
                    },
                    status=status.HTTP_200_OK,
                )
            else:
                return Response(
                    {
                        "status": f"Share permissions for {username_to_share_with} removed."
                    },
                    status=status.HTTP_200_OK,
                )
        else:
...
def get_version_content(request, pk, version_number):
    ...
    try:
        ...
        return Response(
            {"error": "Version content not found"}, status=status.HTTP_404_NOT_FOUND
        )
    except Exception as e:
        print(f"Error in get_version_content: {e}")
        return Response(
            {"error": f"Error retrieving version content: {e}"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
