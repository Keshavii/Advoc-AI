...
def serialize_comment(comment):
    ...
    serialized = {
        ...
        "created_at": (
            comment.get("created_at").isoformat()
            if isinstance(comment.get("created_at"), datetime.datetime)
            else comment.get("created_at")
        ),
        "parent_comment": (
            str(comment["parent_comment"]) if comment.get("parent_comment") else None
        ),
...
def get_comments_for_document(document_id):
    ...
    all_raw_comments = list(
        comments_collection.find({"document_id": document_id}).sort("created_at", 1)
    )

    # Serialize all comments first
    all_serialized_comments = [
        serialize_comment(comment) for comment in all_raw_comments
    ]
