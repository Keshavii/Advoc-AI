...
class DocumentConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if message_type == "new_comment":
            ...
            await self.send(
                text_data=json.dumps({"type": "comments_list", "comments": comments})
            )
        elif message_type == "document_content_change":
            print(
                f"Backend: Broadcasting document_content_change to group {self.document_group_name}"
            )  # Debug log
    ...
    async def document_content_change(self, event):
        ...
        if self.channel_name != event["sender_channel_name"]:
            print(
                f"Backend: Sending document_content_change to channel {self.channel_name}"
            )  # Debug log
            await self.send(
                text_data=json.dumps(
                    {"type": "document_content_change", "content": event["content"]}
                )
            )
