...
class ShareLinkAPITestCase(TestCase):
    ...
    def test_generate_share_link_existing_document_success(
        self, mock_update_share_permissions
    ):
        ...
        response = self.client.post(
            "/api/documents/generate-share-link/",
            {"document_id": document_id, "permission_level": "view"},
        ...
        mock_update_share_permissions.assert_called_once_with(
            document_id, {"permission_level": "view"}
        )

    @patch("documents.views.update_share_permissions")
    def test_generate_share_link_existing_document_failure(
        self, mock_update_share_permissions
    ):
        ...
        response = self.client.post(
            "/api/documents/generate-share-link/",
            {"document_id": document_id, "permission_level": "edit"},
            content_type="application/json",
        )
        ...
        mock_update_share_permissions.assert_called_once_with(
            document_id, {"permission_level": "edit"}
        )
