...
def get_db():
    ...
    )  # The database name is part of the connection string
    return db
...
def get_all_conversations(user=None):
    ...
    try:
        ...
        if user:
            ...
            query = {"$or": [{"owner": user}, {"shared_with_users.username": user}]}
...
def save_conversation(
    ...
    try:
        conversation_doc = {
            ...
            "shared_with_users": (
                shared_with_users if shared_with_users is not None else []
            ),  # New field
        }
        ...
        if document_versions:
            print(
                f"[DEBUG] Initial version (0) content length: {len(document_versions[0]['content'])}"
            )
...
def update_conversation(
    ...
    if existing_conv:
        ...
        if "document_versions" in existing_conv and existing_conv["document_versions"]:
            next_version_number = (
                max(v["version_number"] for v in existing_conv["document_versions"]) + 1
            )
        ...
        if new_document_content is not None:
            ...
            print(
                f"[DEBUG] Pushing new version entry: Version {new_version_entry['version_number']}, Content length: {len(new_version_entry['content'])}"
            )
        else:
    ...
    try:
        result = conversations_collection.update_one(
            {"_id": ObjectId(conversation_id)}, update_doc
        )
        print(
            f"[DEBUG] MongoDB update result: Matched {result.matched_count}, Modified {result.modified_count}"
        )
        return True
    except Exception as e:
    ...
    try:
        ...
        if permission_level:  # Add or update if permission_level is provided
            shared_with_users.append(
                {"username": username, "permission_level": permission_level}
            )
...
def get_document_version_content(conversation_id, version_number):
    ...
    try:
        ...
        if (
            conversation
            and "document_versions" in conversation
            and conversation["document_versions"]
        ):
...
def delete_document_version(conversation_id, version_number):
    ...
    try:
        ...
        if result.modified_count > 0:
            ...
            conversation = conversations_collection.find_one(
                {"_id": ObjectId(conversation_id)}, {"document_versions": 1}
            )
            if conversation and (
                not "document_versions" in conversation
                or not conversation["document_versions"]
            ):
                # If no versions remain, delete the entire conversation
                delete_conversation(conversation_id)
                print(
                    f"Conversation {conversation_id} deleted as no versions remained."
                )
            return True
        return False
    except Exception as e:
