...
try:
    ...
    logging.warning(
        "False positive prevention framework not available - using legacy logic"
    )
...
def extract_clause_with_context(
    text: str, match_pos: int, context_chars: int = 300
) -> str:
    """
...
def calculate_contextual_risk_score(
    base_score: int, text: str, requires_context: List[str], excludes_context: List[str]
) -> Tuple[int, float]:
    """
    ...
    if requires_context:
        for req in requires_context:
            if req and req.lower() in text_lower:
                required_found += 1
        context_ratio = (
            required_found / len(requires_context) if requires_context else 1.0
        )
...
def detect_enhanced_risks(text: str, max_clauses: int = 10) -> List[Dict[str, Any]]:
    ...
    seen_positions: List[Tuple[int, int]] = (
        []
    )  # Track (start, end) positions to avoid overlaps
    ...
    for pattern_obj in ENHANCED_RISK_PATTERNS:
        if pattern_obj.is_regex:
            ...
            pattern = (
                pattern_obj.pattern
                if pattern_obj.case_sensitive
                else pattern_obj.pattern.lower()
            )
        ...
        for match in matches:
            ...
            clause_text = extract_clause_with_context(
                text, match_start, context_chars=350
            )
            ...
            risk = {
                ...
                "clause_text_short": clause_text[
                    :500
                ],  # Shortened for display if needed
            ...
            if FP_FRAMEWORK_AVAILABLE:
                try:
                    fp_score, fp_confidence, balance_type, reasons = (
                        analyze_clause_balance(
                            clause_text, pattern_obj.category.value, adjusted_score
                        )
                    )

                    # If framework detects strong balancing, use its analysis
                    if (
                        balance_type.value in ["mutual", "reciprocal"]
                        and fp_score < adjusted_score
                    ):
    ...
    detected_risks.sort(
        key=lambda x: (x["confidence"], x["risk_score"], x["weight"]), reverse=True
    )
        ...
        for existing_clause in merged:
            if is_duplicate(clause_text, existing_clause.get("clause_text", "")):
                ...
                if clause.get("confidence", 0) > existing_clause.get("confidence", 0):
                    ...
                    existing_clause["category"] = clause.get(
                        "category", existing_clause.get("category")
                    )
                break
