...
BALANCING_INDICATORS_BY_CATEGORY = {
    ...
    "liability": [
        BalancingIndicator(
            patterns=[
                r"mutual\s+(?:liability|indemnification)",
                r"each\s+party.*indemnify",
                r"reciprocal",
            ],
            balance_type=BalanceType.MUTUAL,
            risk_reduction=5,
    ...
    "amendment": [
        BalancingIndicator(
            patterns=[
                r"mutual\s+(?:consent|agreement)",
                r"both\s+parties.*agree",
                r"written\s+agreement.*parties",
            ],
            balance_type=BalanceType.MUTUAL,
            risk_reduction=5,
            confidence_boost=0.4,
        ...
        BalancingIndicator(
            patterns=[
                r"\d{30,}\s*days.*notice",
                r"advance\s+notice",
                r"prior.*notification",
            ],
            balance_type=BalanceType.QUALIFIED,
            risk_reduction=2,
            confidence_boost=0.15,
    ...
    "payment": [
        ...
        BalancingIndicator(
            patterns=[
                ...
                r"both\s+parties.*consent",
                r"negotiated.*good\s+faith",
            ],
            balance_type=BalanceType.MUTUAL,
            risk_reduction=3,
            confidence_boost=0.25,
    ...
    "confidentiality": [
        BalancingIndicator(
            patterns=[
                r"mutual\s+(?:confidentiality|NDA)",
                r"each\s+party.*confidential",
                r"reciprocal",
            ],
            balance_type=BalanceType.MUTUAL,
            risk_reduction=4,
    ...
    "dispute_resolution": [
        BalancingIndicator(
            patterns=[
                r"mutual\s+(?:arbitration|mediation)",
                r"agreed\s+arbitrator",
                r"both\s+parties.*consent",
            ],
            balance_type=BalanceType.MUTUAL,
            risk_reduction=3,
...
def analyze_clause_balance(
    clause_text: str, category: str, base_risk_score: int
...
def detect_identical_replacement(
    original_clause: str, replacement_clause: str, threshold: float = 0.90
) -> bool:
    """
...
def should_filter_clause(
    clause: Dict, min_risk_score: int = 3, min_confidence: float = 0.4
) -> Tuple[bool, str]:
    """
...
def apply_false_positive_filters(
    ...
    for clause in clauses:
        should_filter, reason = should_filter_clause(
            clause, min_risk_score, min_confidence
        )
...
def get_balancing_examples_for_prompt(category: str) -> str:
    ...
    examples.append(
        "- One-sided language (Provider/Vendor may... but Customer must...)"
    )
...
if __name__ == "__main__":
    ...
    score, conf, balance, reasons = analyze_clause_balance(
        balanced_termination, "termination", base_risk_score=5
    )
    ...
    score2, conf2, balance2, reasons2 = analyze_clause_balance(
        one_sided_termination, "termination", base_risk_score=5
    )
