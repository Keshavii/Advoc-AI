...
def analyze_document_async(self, session_id, document_text):
    ...
    try:
        ...
        cache.set(
            ...
            {"status": "processing", "progress": 10, "message": "Starting analysis..."},
            timeout=3600,
        )
        ...
        cache.set(
            f"task_status:{session_id}",
            {"status": "processing", "progress": 80, "message": "Saving results..."},
            timeout=3600,
        )
        ...
        logger.error(
            f"Error in async analysis for session {session_id}: {exc}", exc_info=True
        )
        ...
        try:
            ...
            try:
                ...
                if session:
                    session.summary = (
                        f"Analysis failed after multiple attempts: {str(exc)}"
                    )
                    session.save()
