...
def refine_clause_solutions_with_patterns_and_llm(
    ...
    if not pattern_info:
        ...
        pattern_info = {
            ...
            "severity": risk_score,
            "solution_template": mitigation_strategies.get(
                "general",
                "Negotiate balanced, mutual terms with clear limits and adequate protections.",
            ),
    ...
    try:
        ...
        chain = refinement_prompt | structured_llm.with_structured_output(
            RefinedSolution
        )

        logger.info(
            f"Invoking Gemini for tailored refinement (pattern: {matched_pattern or 'general'})"
        )
        result = chain.invoke(
            {
        ...
        if result is None:
            logger.warning(
                "Gemini returned None for refinement - using pattern templates"
            )
        ...
        try:
            ...
            logger.warning(
                f"Failed to extract data from Gemini result: {extract_error}"
            )
            raise ValueError(f"Failed to extract refinement data: {extract_error}")

        logger.info(
            f"Gemini refinement result - mitigation length: {len(refined.get('mitigation', ''))}, replacement length: {len(refined.get('replacement_clause', ''))}"
        )
        ...
        if refined.get("mitigation") and len(refined["mitigation"]) > 30:
            ...
            logger.info(
                f"✅ Using Gemini negotiation strategy (mitigation): {refined['mitigation'][:80]}..."
            )
        else:
            # Fallback to pattern template
            clause["mitigation"] = pattern_info["solution_template"]
            logger.warning(
                f"⚠️ Gemini negotiation strategy too short ({len(refined.get('mitigation', ''))} chars), using pattern template"
            )

        if (
            refined.get("replacement_clause")
            and len(refined["replacement_clause"]) > 50
        ):
            clause["replacement_clause"] = refined["replacement_clause"]
            logger.info(
                f"✅ Using Gemini alternative clause text (replacement): {refined['replacement_clause'][:80]}..."
            )
        else:
            # Fallback to pattern template
            clause["replacement_clause"] = pattern_info["alternative_pattern"]
            logger.warning(
                f"⚠️ Gemini alternative clause too short ({len(refined.get('replacement_clause', ''))} chars), using pattern template"
            )
        ...
        clause["refinement_method"] = (
            "pattern_plus_llm" if matched_pattern else "llm_general"
        )
...
def batch_refine_clauses(
    ...
    sorted_indexed = sorted(
        indexed_clauses, key=lambda x: x[1].get("risk_score", 0), reverse=True
    )
    ...
    for idx, clause in to_refine_indexed:
        try:
            ...
            logger.info(
                f"Refined clause {idx} (risk_score: {clause.get('risk_score')})"
            )
        except Exception as exc:
