...
def generate_dynamic_alternative_clause(
    original_clause: str, doc_type: str, risk_category: str, clause_context: Dict
) -> str:
    """
    ...
    if risk_category in patterns:
        ...
        if doc_type == "service_agreement":
            ...
            template = template.replace(
                "[EXCLUSIONS]",
                "is publicly available, was independently developed, or was already known to the receiving party",
            )
            ...
            template = template.replace(
                "[SPECIFIC ACTIVITY]", "providing substantially similar services"
            )
            template = template.replace(
                "[SPECIFIC PRODUCTS/SERVICES]", "Company's core products"
            )
