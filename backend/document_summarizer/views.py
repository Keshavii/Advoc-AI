...
def _strip_section_header(text: str) -> str:
    ...
    text = re.sub(
        r"^\s*(Section|Article|Clause|Paragraph)\s+[IVXLCDM\d]+[\.\:)]*\s*",
        "",
        text,
        flags=re.IGNORECASE,
    )
...
def _find_best_match(
    full_text: str, snippet: str, min_match_length: int = 30
) -> Tuple[int, int]:
    """
    ...
    words = [w for w in snippet_for_words.split() if len(w) > 3][
        :10
    ]  # First 10 significant words
...
def _build_highlighted_preview(
    full_text: str, clauses: List[Dict[str, Any]]
    ...
    matches: List[Tuple[int, int, int, int]] = (
        []
    )  # (start, end, risk_score, clause_index)
    successfully_highlighted: List[int] = (
        []
    )  # Track which clause indices were highlighted
    expanded_clause_texts: Dict[int, str] = (
        {}
    )  # Map clause_idx -> expanded sentence text
    lower_text = full_text.lower()
    ...
    for clause_idx, clause in enumerate(clauses):
                    ...
                    if start_index != -1:
                        ...
                        if len(middle) > 40:
                            ...
                            if start_index != -1:
                                ...
                                logger.info(
                                    f"Found using middle match: '{middle[:40]}...'"
                                )
        ...
        start_index, end_index = _expand_to_sentence_boundary(
        ...
        logger.info(
            f"Expanded clause to sentence boundaries: [{start_index}:{end_index}] = '{expanded_text[:60]}...'"
        )
        ...
        for i, (existing_start, existing_end, existing_risk, existing_idx) in enumerate(
            matches
        ):
            ...
            overlap_of_current = (
                overlap_length / current_length if current_length > 0 else 0
            )
            overlap_of_existing = (
                overlap_length / existing_length if existing_length > 0 else 0
            )
            ...
            if overlap_of_current > 0.7 and overlap_of_existing > 0.7:
                ...
                if risk_score > existing_risk:
                    ...
                    logger.info(
                        f"Duplicate clause detected (overlap {overlap_of_current:.0%}/{overlap_of_existing:.0%}): replacing with higher risk"
                    )
                else:
                    ...
                    logger.info(
                        f"Duplicate clause detected (overlap {overlap_of_current:.0%}/{overlap_of_existing:.0%}): keeping existing higher risk"
                    )
                ...
                logger.info(
                    f"Adjacent clauses with minor overlap ({overlap_length} chars): keeping both separate"
                )
    ...
    if not matches:
        logger.warning(
            f"No clauses could be highlighted from {len(clauses)} detected risks"
        )
        return html.escape(full_text).replace("\n", "<br />"), []
    ...
    for start_index, end_index, risk_score, clause_idx in matches:
        ...
        risk_level = (
            "high" if risk_score >= 4 else "medium" if risk_score >= 3 else "low"
        )
    ...
    logger.info(
        f"Successfully highlighted {len(successfully_highlighted)} out of {len(clauses)} clauses"
    )
    return (
        highlighted_html.replace("\n", "<br />"),
        list(set(successfully_highlighted)),
        expanded_clause_texts,
    )


def _generate_mock_analysis(
    full_text: str, preview_excerpt: str, truncated_document: str
) -> Dict[str, Any]:
    ...
    highlighted_preview = (
        html.escape(safe_full_text).replace("\n", "<br />") if safe_full_text else ""
    )
    highlighted_indices = []
    expanded_texts = {}
    ...
    return {
        ...
        or "AI summarization is unavailable without a configured Gemini API key.",
        "high_risk_clauses": [],
...
RISK_KEYWORDS: List[Dict[str, Any]] = [
    {
        "pattern": "shall indemnify",
        "rationale": "Broad indemnity can transfer extensive liability to you.",
        "weight": 5,
        "default_score": 5,
        "suggestion": "Limit indemnity to third-party losses caused by the indemnifying party and cap recoverable damages.",
        "replacement_category": "indemnity",
    },
    {
        "pattern": "indemnify",
        "rationale": "Indemnification language often shifts responsibility for losses.",
        "weight": 4,
        "default_score": 5,
        "suggestion": "Seek mutual indemnities and restrict scope to negligence or breach documented by the indemnifying party.",
        "replacement_category": "indemnity",
    },
    {
        "pattern": "indemnity",
        "rationale": "Indemnity terms may create unlimited loss exposure.",
        "weight": 4,
        "default_score": 5,
        "suggestion": "Tie indemnity to specific, provable harms and add financial caps or insurance requirements.",
        "replacement_category": "indemnity",
    },
    {
        "pattern": "hold harmless",
        "rationale": "Hold harmless obligations can expand your liability exposure.",
        "weight": 4,
        "default_score": 5,
        "suggestion": "Convert hold harmless language to mutual indemnities or limit to direct damages from proven misconduct.",
        "replacement_category": "indemnity",
    },
    {
        "pattern": "defend and indemnify",
        "rationale": "Defense obligations add cost and litigation risk.",
        "weight": 4,
        "default_score": 5,
        "suggestion": "Remove the duty to defend or make reimbursement contingent on proven liability and reasonable costs.",
        "replacement_category": "indemnity",
    },
    {
        "pattern": "limitation of liability",
        "rationale": "Liability caps may be one-sided or exclude key damages.",
        "weight": 4,
        "default_score": 4,
        "suggestion": "Ensure liability caps are mutual, tied to fees, and carve out critical losses like data breaches or IP infringement.",
        "replacement_category": "liability_cap",
    },
    {
        "pattern": "limitation on liability",
        "rationale": "Liability caps may be one-sided or exclude key damages.",
        "weight": 4,
        "default_score": 4,
        "suggestion": "Increase the liability cap to a realistic amount and add carve-outs for gross negligence or willful misconduct.",
        "replacement_category": "liability_cap",
    },
    {
        "pattern": "liability shall not exceed",
        "rationale": "Dollar caps on liability can be too low for the risk.",
        "weight": 4,
        "default_score": 4,
        "suggestion": "Link the liability cap to total contract value or annual fees and carve out critical categories of harm.",
        "replacement_category": "liability_cap",
    },
    {
        "pattern": "limitation on remedies",
        "rationale": "Limits on remedies may prevent adequate recourse.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Add supplemental remedies or exceptions when the primary remedy fails to deliver the contracted outcome.",
        "replacement_category": "remedies",
    },
    {
        "pattern": "exclusive remedy",
        "rationale": "Exclusive remedy clauses restrict available recourse.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Allow alternative remedies if the exclusive remedy fails or if breaches are material or repeated.",
        "replacement_category": "remedies",
    },
    {
        "pattern": "waiver of consequential",
        "rationale": "Waives indirect damages that may be necessary to recover.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Carve out consequential damages arising from the other party's breach, data loss, or IP infringement.",
        "replacement_category": "remedies",
    },
    {
        "pattern": "warranty disclaimer",
        "rationale": "Warranty disclaimers can remove important protections.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Add baseline performance warranties or acceptance testing to ensure minimum service standards.",
        "replacement_category": "warranty",
    },
    {
        "pattern": "provided on an as-is basis",
        "rationale": "As-is language may waive key warranties.",
        "weight": 3,
        "default_score": 3,
        "suggestion": "Insert performance commitments or a right to terminate if the deliverable fails agreed specifications.",
        "replacement_category": "warranty",
    },
    {
        "pattern": "liquidated damages",
        "rationale": "Preset damages may be punitive or costly.",
        "weight": 4,
        "default_score": 4,
        "suggestion": "Verify liquidated damages align with actual anticipated loss and cap total exposure.",
        "replacement_category": "penalties",
    },
    {
        "pattern": "early termination fee",
        "rationale": "Termination penalties can be financially burdensome.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Negotiate prorated termination fees tied to unrecovered costs or limit the fee to a short notice period.",
        "replacement_category": "penalties",
    },
    {
        "pattern": "penalty",
        "rationale": "Penalty provisions may create significant financial exposure.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Replace punitive penalties with reasonable liquidated damages or cure rights before charges apply.",
        "replacement_category": "penalties",
    },
    {
        "pattern": "termination for convenience",
        "rationale": "Termination for convenience without notice harms contract stability.",
        "weight": 4,
        "default_score": 4,
        "suggestion": "Add reasonable notice periods, mutual termination rights, and reimbursement of non-recoverable costs.",
        "replacement_category": "termination",
    },
    {
        "pattern": "terminate at any time",
        "rationale": "Unqualified termination rights create uncertainty.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Require advance notice, minimum commitment, or limit termination to defined trigger events.",
        "replacement_category": "termination",
    },
    {
        "pattern": "automatic renewal",
        "rationale": "Automatic renewal clauses can extend obligations without explicit consent.",
        "weight": 4,
        "default_score": 4,
        "suggestion": "Shorten the renewal term, add renewal reminders, and allow opt-out with reasonable notice.",
        "replacement_category": "renewal",
    },
    {
        "pattern": "auto-renew",
        "rationale": "Automatic renewal clauses can extend obligations without explicit consent.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Require written confirmation for renewal and reduce advance notice requirements.",
        "replacement_category": "renewal",
    },
    {
        "pattern": "perpetual term",
        "rationale": "Perpetual terms lock parties into long commitments.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Add periodic renewal checkpoints or a right to terminate without penalty after an initial term.",
        "replacement_category": "renewal",
    },
    {
        "pattern": "arbitration",
        "rationale": "Mandatory arbitration impacts dispute resolution rights.",
        "weight": 3,
        "default_score": 3,
        "suggestion": "Ensure arbitration rules, venue, and costs are balanced and preserve access to courts for injunctive relief.",
        "replacement_category": "dispute_resolution",
    },
    {
        ...
        "weight": 3,
        "default_score": 3,
        "suggestion": "Make arbitration mutual, choose a neutral forum, and allow appeals for manifest error or injunctive relief in court.",
        "replacement_category": "dispute_resolution",
    },
    {
        "pattern": "waiver of jury trial",
        "rationale": "Waiving jury trial limits litigation options.",
        "weight": 3,
        "default_score": 3,
        "suggestion": "Consider removing the waiver or ensure it is mutual and limited to defined dispute types.",
        "replacement_category": "dispute_resolution",
    },
    {
        "pattern": "governing law",
        "rationale": "Governing law in an unfavorable venue can affect outcomes.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Renegotiate governing law to a neutral or home jurisdiction that aligns with your legal protections.",
        "replacement_category": "jurisdiction",
    },
    {
        "pattern": "exclusive jurisdiction",
        "rationale": "Exclusive jurisdiction may create unfavorable litigation venues.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Allow litigation in your local courts or agree to a mutually convenient jurisdiction.",
        "replacement_category": "jurisdiction",
    },
    {
        "pattern": "venue shall be",
        "rationale": "Fixed venue may require litigating in distant courts.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Amend venue provision to include your jurisdiction or permit remote dispute resolution.",
        "replacement_category": "jurisdiction",
    },
    {
        "pattern": "limitation period",
        "rationale": "Reduced limitation periods can curtail legal remedies.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Extend limitation periods to statutory defaults or a timeframe that reflects the transaction risk.",
        "replacement_category": "jurisdiction",
    },
    {
        "pattern": "confidentiality",
        "rationale": "Strict confidentiality clauses may impose burdensome restrictions.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Ensure confidentiality is mutual, includes standard carve-outs, and limits survival to a reasonable duration.",
        "replacement_category": "confidentiality",
    },
    {
        "pattern": "non-disclosure",
        "rationale": "Strict confidentiality clauses may impose burdensome restrictions.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Add carve-outs for legal disclosures, advisors, and information already known or independently developed.",
        "replacement_category": "confidentiality",
    },
    {
        "pattern": "unilateral amendment",
        "rationale": "Unilateral amendment rights allow one party to change terms without consent.",
        "weight": 4,
        "default_score": 4,
        "suggestion": "Require mutual agreement or give the non-amending party termination rights if changes are unacceptable.",
        "replacement_category": "amendment",
    },
    {
        "pattern": "may modify this agreement",
        "rationale": "Allowing unilateral changes adds significant uncertainty.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Add prior notice requirements and allow rejection of material changes without penalty.",
        "replacement_category": "amendment",
    },
    {
        "pattern": "sole discretion",
        "rationale": "Sole discretion terms often grant broad unilateral power.",
        "weight": 3,
        "default_score": 3,
        "suggestion": "Qualify discretion so it may not be unreasonably withheld, conditioned, or delayed.",
        "replacement_category": "discretion",
    },
    {
        "pattern": "sole and absolute discretion",
        "rationale": "Absolute discretion eliminates checks and balances.",
        "weight": 4,
        "default_score": 4,
        "suggestion": "Replace with objective criteria or require written consent that cannot be unreasonably withheld.",
        "replacement_category": "discretion",
    },
    {
        "pattern": "assignment without consent",
        "rationale": "Assignments without consent can shift obligations to unknown parties.",
        "weight": 3,
        "default_score": 3,
        "suggestion": "Limit assignment to affiliates meeting financial criteria or require prior written consent.",
        "replacement_category": "assignment",
    },
    {
        "pattern": "non-compete",
        ...
        "weight": 3,
        "default_score": 3,
        "suggestion": "Narrow the non-compete scope, geography, and term or remove it entirely if unnecessary.",
        "replacement_category": "competition",
    },
    {
        "pattern": "non-solicitation",
        "rationale": "Non-solicitation clauses can hinder hiring or client outreach.",
        "weight": 2,
        "default_score": 2,
        "suggestion": "Limit non-solicitation to direct poaching of employees or clients for a short duration.",
        "replacement_category": "competition",
    },
    {
        "pattern": "intellectual property shall belong",
        "rationale": "Transfers of IP ownership may be unfavorable.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Pursue joint ownership or retain ownership with a license grant tailored to the project.",
        "replacement_category": "ip",
    },
    {
        "pattern": "assign all intellectual property",
        "rationale": "Broad IP assignments can forfeit key rights.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Restrict assignments to developed deliverables and secure a perpetual license-back.",
        "replacement_category": "ip",
    },
    {
        "pattern": "license is revocable",
        "rationale": "Revocable licenses may undercut usage rights.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Negotiate for an irrevocable, perpetual license that survives termination if fees are paid.",
        "replacement_category": "ip",
    },
    {
        "pattern": "data breach",
        "rationale": "Data breach responsibilities may impose heavy obligations.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Define security standards, notification timelines, and cost sharing for breach response.",
        "replacement_category": "data_security",
    },
    {
        "pattern": "personally identifiable information",
        "rationale": "PII handling clauses may create compliance burdens.",
        "weight": 3,
        "default_score": 3,
        "suggestion": "Clarify data protection requirements and ensure the other party maintains compliant safeguards.",
        "replacement_category": "data_security",
    },
    {
        "pattern": "security incident",
        "rationale": "Security incident obligations can be costly or strict.",
        "weight": 3,
        "default_score": 3,
        "suggestion": "Set reasonable response windows, cooperation obligations, and limits on indemnified costs.",
        "replacement_category": "data_security",
    },
    {
        "pattern": "service level credit",
        "rationale": "SLA credits can accumulate and erode revenue.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Cap service credits, allow cure periods, and limit credits to a percentage of monthly fees.",
        "replacement_category": "sla",
    },
    {
        "pattern": "uptime commitment",
        "rationale": "Aggressive uptime commitments may be hard to meet.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Adjust uptime targets to achievable levels and include maintenance windows and exclusions.",
        "replacement_category": "sla",
    },
    {
        "pattern": "most favored nation",
        "rationale": "MFN clauses force matching best pricing offered to others.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Limit MFN to similarly situated customers, term-bound periods, and confidential comparison data.",
        "replacement_category": "pricing",
    },
    {
        "pattern": "exclusive dealing",
        "rationale": "Exclusive dealing can block other partnerships.",
        "weight": 3,
        "default_score": 3,
        "suggestion": "Restrict exclusivity to defined products or regions and allow exceptions for key partners.",
        "replacement_category": "exclusivity",
    },
    {
        "pattern": "change of control",
        "rationale": "Change-of-control triggers can terminate the agreement.",
        "weight": 3,
        "default_score": 3,
        "suggestion": "Replace automatic termination with notice and opportunity to cure or maintain assignment rights.",
        "replacement_category": "change_control",
    },
    {
        "pattern": "disclaimer of liability",
        "rationale": "Broad liability disclaimers remove recourse for damages.",
        "weight": 3,
        "default_score": 4,
        "suggestion": "Add carve-outs for gross negligence, willful misconduct, data loss, and confidentiality breaches.",
        "replacement_category": "disclaimer",
    },
    {
        "pattern": "no consequential damages",
        "rationale": "Excluding consequential damages limits recovery options.",
        "weight": 3,
        "default_score": 3,
        "suggestion": "Carve out consequential damages arising from the other party's breach or data loss.",
        "replacement_category": "disclaimer",
    },
    {
        "pattern": "set-off rights",
        "rationale": "Set-off allows withholding payments owed to you.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Limit set-off to undisputed amounts and require prior written notice of intent to offset.",
        "replacement_category": "payments",
    },
    {
        "pattern": "late payment interest",
        "rationale": "Excessive late fees create punitive financial exposure.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Cap late fees at statutory limits and include a short grace period for payment processing.",
        "replacement_category": "payments",
    },
    {
        "pattern": "costs and expenses",
        "rationale": "Obligations to cover all costs and expenses add liability.",
        "weight": 3,
        "default_score": 3,
        "suggestion": "Require pre-approval for expenses, limit to reasonable amounts, and demand documentation.",
        "replacement_category": "payments",
    },
    {
        "pattern": "waives all claims",
        "rationale": "Waiving claims may eliminate legitimate remedies.",
        "weight": 4,
        "default_score": 4,
        "suggestion": "Narrow the waiver to known claims or limit it to liabilities arising before the agreement date.",
        "replacement_category": "waivers",
    },
    {
        "pattern": "force majeure",
        "rationale": "Force majeure carve-outs can cause performance issues.",
        "weight": 2,
        "default_score": 2,
        "suggestion": "Clarify notice, mitigation obligations, and rights to suspend or terminate after extended events.",
        "replacement_category": "force_majeure",
    },
    {
        "pattern": "governing language",
        "rationale": "Language precedence may affect interpretation.",
        "weight": 1,
        "default_score": 2,
        "suggestion": "Confirm the governing language matches the negotiated version or include certified translations.",
        "replacement_category": "language",
    },
    {
        "pattern": "compliance with all laws",
        "rationale": "Broad compliance obligations may be difficult to satisfy.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Limit compliance covenant to laws applicable to the services and add knowledge or control qualifiers.",
        "replacement_category": "compliance",
    },
    {
        "pattern": "insurance certificates",
        "rationale": "Extensive insurance requirements increase costs.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Align insurance limits with industry standards and allow proof upon reasonable request.",
        "replacement_category": "insurance",
    },
    {
        "pattern": "audit rights",
        "rationale": "Audit rights grant access to records and facilities.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Limit audit frequency, require advance notice, and impose confidentiality on audit findings.",
        "replacement_category": "audit",
    },
    {
        "pattern": "escrow account",
        "rationale": "Escrow obligations tie up funds or IP.",
        "weight": 2,
        "default_score": 3,
        "suggestion": "Clarify release conditions, cost sharing, and the scope of assets placed in escrow.",
        "replacement_category": "escrow",
    },
    {
        "pattern": "liability",
        "rationale": "General liability language can indicate elevated risk.",
        "weight": 1,
        "default_score": 2,
        "suggestion": "Review surrounding language to ensure liability responsibilities are balanced and clearly defined.",
        "replacement_category": "generic",
    },
]
...
def _normalize_clause_structure(raw_clause: Dict[str, Any]) -> Dict[str, Any]:
    clause_text = (
        raw_clause.get("clause_text") or raw_clause.get("clauseText") or ""
    ).strip()
    if not clause_text:
        return {}
    ...
    risk_level = (
        raw_clause.get("risk_level") or raw_clause.get("riskLevel") or ""
    ).strip()
    if not risk_level:
...
def _order_clauses_by_priority(
...
def _fallback_risk_clauses(full_text: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    for risk in detected_risks:
        ...
        replacement_clause = DEFAULT_REPLACEMENTS.get(
            category, DEFAULT_REPLACEMENTS["generic"]
        )
    ...
    if not enhanced_clauses:
        logger.warning(
            "Enhanced detection found no risks, using legacy keyword matching"
        )
            ...
            if matches:
                ...
                if snippet:
                    clauses.append(
                        {
                            "clause_text": snippet,
                            "risk_level": _score_to_label(
                                keyword_info.get("default_score", 3)
                            ),
...
def _chunk_document(
    full_text: str, chunk_size: int = 2500, overlap: int = 300
) -> List[Dict[str, Any]]:
...
def _dedupe_clauses(
    clauses: List[Dict[str, Any]], limit: int = 8
) -> List[Dict[str, Any]]:
    ...
    for clause in clauses:
        ...
        if fingerprint in seen:
            ...
            if normalized_clause.get("risk_score", 0) > existing_clause.get(
                "risk_score", 0
            ):
        ...
        if clause_pos:
            for i, existing_clause in enumerate(deduped):
                ...
                if existing_pos:
                    ...
                    if clause_length > 0 and (
                        overlap > clause_length * 0.5 or overlap > existing_length * 0.5
                    ):
                        is_duplicate = True
                        # Keep higher risk score
                        if normalized_clause.get("risk_score", 0) > existing_clause.get(
...
def _generate_comprehensive_summary(
    full_text: str, doc_type: str, llm, doc_type_name: str, use_llm: bool = True
) -> Dict[str, Any]:
    ...
    try:
        ...
        class PartyInfo(BaseModel):
            ...
            role: str = Field(
                ...,
                description="Role in document (e.g., Employer, Tenant, Service Provider, Disclosing Party)",
            )
            simple_explanation: Optional[str] = Field(
                None,
                description="Plain language explanation of what this party does in this agreement",
            )
        ...
        class FinancialTerm(BaseModel):
            ...
            simple_explanation: Optional[str] = Field(
                None, description="Plain language explanation"
            )
        ...
        class ComprehensiveSummary(BaseModel):
            ...
            execution_date: Optional[str] = Field(
                None, description="Date document was/will be signed"
            )
            parties: List[PartyInfo] = Field(
                ..., description="All parties involved with roles"
            )
            purpose: str = Field(
                ..., description="Core reason this document exists (2-3 sentences max)"
            )
            key_obligations: Dict[str, str] = Field(
                ..., description="Map of party name to their main responsibilities"
            )
            financial_terms: List[FinancialTerm] = Field(
                default_factory=list, description="Payment amounts, fees, compensation"
            )
            term_and_termination: TerminationInfo = Field(
                ..., description="Duration and how to end the agreement"
            )
            compliance_requirements: Optional[List[str]] = Field(
                default_factory=list, description="Legal compliance obligations"
            )
            important_deadlines: Optional[List[str]] = Field(
                default_factory=list, description="Time-sensitive obligations"
            )
            attachments_mentioned: Optional[List[str]] = Field(
                default_factory=list,
                description="Schedules, exhibits, annexures referenced",
            )
            legal_terms_explained: List[LegalTermExplanation] = Field(
                default_factory=list,
                description="Complex legal terms with plain language meanings",
            )
            executive_summary: str = Field(
                ...,
                description="2-3 paragraph plain language overview suitable for non-lawyers (150-250 words)",
            )
        ...
        fallback_models = ["gemini-1.5-flash"]  # Fallback option

        logger.info(
            f"Using {model_for_summary} for comprehensive summary (optimized for quota efficiency)"
        )
        ...
        if len(full_text) > 6000:
            ...
            truncated_text += (
                ...
                + str(len(full_text) - 6000)
                + " more characters...]\\n\\nNote: Analyze the provided excerpt and extract all available information."
            )

        logger.info(
            f"Generating comprehensive {doc_type_name} summary using LLM (text length: {len(truncated_text)} chars)..."
        )
        ...
        for model_name in models_to_try:
            ...
            try:
                ...
                current_structured_llm = current_llm.with_structured_output(
                    ComprehensiveSummary
                )
            ...
            for attempt in range(max_attempts):
                try:
                    logger.info(
                        f"Model {model_name}, attempt {attempt + 1}/{max_attempts}..."
                    )
                    ...
                    result = current_chain.invoke(
                        ...
                        config={
                            "max_retries": 0,
                            "request_timeout": 60,
                        },  # No retries here, we handle it ourselves
                    )

                    # Check immediately if result is None
                    if result is None:
                        logger.warning(
                            f"{model_name} returned None on attempt {attempt + 1}"
                        )
                        if attempt < max_attempts - 1:
                            ...
                            logger.warning(
                                f"{model_name} returned None after {max_attempts} attempts, trying next model"
                            )
                    ...
                    if (
                        not hasattr(result, "model_dump")
                        and not hasattr(result, "dict")
                        and not isinstance(result, dict)
                    ):
                        logger.warning(
                            f"{model_name} returned unexpected type: {type(result)}"
                        )
                        if attempt < max_attempts - 1:
                            time.sleep(2)
                            continue
                            ...
                            logger.warning(
                                f"{model_name} returned wrong type after {max_attempts} attempts, trying next model"
                            )
                    ...
                    logger.info(
                        f"✅ {model_name} returned valid result on attempt {attempt + 1}"
                    )
                    ...
                    logger.warning(
                        f"{model_name} attempt {attempt + 1} failed: {error_msg}"
                    )
                    ...
                    if attempt < max_attempts - 1:
                        ...
                        logger.warning(
                            f"{model_name} failed after {max_attempts} attempts, trying next model"
                        )
                        break  # Try next model
        ...
        if result is None:
            error_message = (
                f"All models failed to generate output. Last error: {last_error}"
            )
        ...
        if result is None:
            logger.error(
                "LLM returned None after all retry attempts - falling back to regex extraction"
            )
            raise ValueError(
                "LLM returned None after retry logic - likely API quota or persistent connection issue"
            )
        ...
        try:
            ...
            logger.error(
                f"Failed to extract data from LLM result: {extract_error}",
                exc_info=True,
            )
        ...
        if not summary_dict.get("executive_summary"):
            summary_dict["executive_summary"] = textwrap.shorten(
                full_text, width=250, placeholder="..."
            )

        logger.info(
            f"✅ Comprehensive summary generated: {len(summary_dict.get('executive_summary', ''))} chars, "
        ...
        if is_quota_issue:
            logger.warning(
                f"LLM quota exceeded for comprehensive summary, using regex fallback: {error_msg}"
            )
        else:
            logger.error(
                f"Comprehensive summary LLM generation failed: {exc}", exc_info=True
            )
        ...
        try:
            ...
            return {
                ...
                "executive_summary": textwrap.shorten(
                    full_text, width=500, placeholder="..."
                ),
                "parties": [],
    ...
    try:
        ...
        for pattern in money_patterns:
            ...
            for amount in amounts[:5]:
                financial_terms.append({"item": "Payment", "amount": amount.strip()})
        ...
        legal_term_keywords = [
            (
                "indemnification",
                "If something goes wrong because of one party, they must pay for any resulting costs",
            ),
            (
                "force majeure",
                "If unexpected events like natural disasters happen, neither party is blamed for delays",
            ),
            (
                "liquidated damages",
                "Pre-agreed penalty amount if someone breaks the contract",
            ),
            (
                "severability",
                "If one part of the contract is invalid, the rest still applies",
            ),
            (
                "governing law",
                "Which state or country's laws control how disputes are resolved",
            ),
            ...
            (
                ...
                "Ownership of ideas, inventions, and creative work",
            ),
            (
                "warranty",
                "Promise or guarantee that something is true or will work as stated",
            ),
        ...
        return {
            ...
            "term_and_termination": {
                ...
                "renewal_terms": None,
                "termination_process": (
                    f"Either party may terminate with {notice_period} notice"
                    if notice_period
                    else "Not specified"
                ),
                "notice_period": notice_period,
                "simple_explanation": (
                    f"This agreement lasts for {duration}. You can exit by giving {notice_period} written notice."
                    if duration and notice_period
                    else "Review document for specific term and termination details."
                ),
            },
        ...
        logger.error(
            f"Failed to extract comprehensive summary from analysis: {exc}",
            exc_info=True,
        )
        return {
            "document_type": doc_type_name,
...
def _analyze_chunk_with_llm(
    ...
    try:
        ...
        logger.warning(
            "Chunk %s analysis failed, using heuristic fallback: %s",
            idx + 1,
            error_message,
        )

        if (
            GoogleModelNotFound and isinstance(exc, GoogleModelNotFound)
...
def _analyze_focus_snippets(
    ...
    from .document_classifier import (
        get_type_specific_system_prompt,
        get_type_specific_examples,
        DOCUMENT_TYPES,
    )
    ...
    if risk_patterns:
        pattern_examples = []
        for risk_category, pattern_info in list(risk_patterns.items())[
            :3
        ]:  # Show top 3 patterns
        ...
        example_messages.extend(
            [
                ...
                (
                    ...
                    '"replacement_clause": "'
                    + example["replacement_clause"][:100]
                    + '..."}}]}}',
                ),
            ]
        )
    ...
    try:
        ...
        error_message = str(exc)
        logger.warning(
            "Focus snippet analysis failed, using heuristic fallback: %s", error_message
        )
...
def generate_document_analysis(text: str) -> Dict[str, Any]:
    ...
    if not settings.GEMINI_API_KEY or not LLM_AVAILABLE:
        if not settings.GEMINI_API_KEY:
            logger.warning(
                "GEMINI_API_KEY not configured; falling back to heuristic analysis."
            )
        elif LLM_LAST_ERROR:
            logger.warning(
                "Gemini model disabled due to previous error: %s", LLM_LAST_ERROR
            )

        analysis = _generate_mock_analysis(
            full_text, preview_excerpt, truncated_document
        )
        if LLM_LAST_ERROR:
    ...
    class ClauseHighlight(BaseModel):
        clause_text: str = Field(
            ...,
            description="Exact clause copied from the chunk that signals elevated risk.",
        )
        risk_score: int = Field(
            ...,
            description="Integer risk score from 1 (minimal) to 5 (critical).",
            ge=1,
            le=5,
        )
        risk_level: str = Field(
            ...,
            description="Risk severity label that aligns with the assigned risk_score.",
        )
        rationale: str = Field(
            ...,
            description="Brief explanation (<=50 words) of why the clause is risky.",
        )
        mitigation: str = Field(
            ...,
            description="Specific revision or negotiation request (<=45 words) to reduce the highlighted risk.",
        )
        replacement_clause: str = Field(
            ...,
            description="Low-risk replacement clause in formal legal language that can substitute the risky clause.",
        )

    class DocumentAnalysis(BaseModel):
        summary: str = Field(
            ..., description="Concise (<=140 words) synopsis of the chunk."
        )
        high_risk_clauses: List[ClauseHighlight] = Field(
            default_factory=list,
            description="Clauses in the chunk that warrant attention.",
        )
    ...
    doc_type_name = DOCUMENT_TYPES.get(doc_type, {}).get("name", "General Agreement")
    logger.info(
        f"Document classified as: {doc_type_name} (confidence: {confidence:.0%})"
    )
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            ...
            ("system", improved_prompts["chunk_instructions"]),
            (
                "system",
...
def generate_document_analysis(text: str) -> Dict[str, Any]:
    ...
    chunk_results: List[Dict[str, Any]] = [None] * len(
        chunks
    )  # Pre-allocate for ordered results

    if not chunks:
        chunks = [{"text": full_text, "start": 0, "end": len(full_text)}]

    keyword_scores = [
        (_keyword_score(chunk["text"]), idx) for idx, chunk in enumerate(chunks)
    ]
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                logger.error(
                    f"Error processing chunk {idx} in parallel: {exc}", exc_info=True
                )
                # Fallback for failed LLM chunk if any
                chunk_results[idx] = {
                    "summary": textwrap.shorten(
                        chunks[idx]["text"].replace("\n", " "),
                        width=260,
                        placeholder="…",
                    ),
                    "high_risk_clauses": _fallback_risk_clauses(
                        chunks[idx]["text"], limit=2
                    ),
                }
    ...
    if not clause_candidates:
        ...
        if clause_candidates:
            summary_parts.append(
                "Enhanced pattern matching detected high-risk clauses."
            )
    else:
            ...
            heuristic_clauses=[
                {
                    ...
                    "mitigation": risk["mitigation"],
                    "replacement_clause": DEFAULT_REPLACEMENTS.get(
                        risk.get("category", "generic"), DEFAULT_REPLACEMENTS["generic"]
                    ),
                    "confidence": risk.get("confidence", 0.75),
    ...
    if SOLUTION_REFINEMENT_AVAILABLE and deduped_clauses:
        logger.info(
            f"Refining {len(deduped_clauses)} clauses with pattern-based templates + Gemini tailoring"
        )
        try:
            ...
            logger.warning(
                f"Solution refinement failed, using original solutions: {exc}"
            )
    else:
        if not SOLUTION_REFINEMENT_AVAILABLE:
            logger.warning(
                "Solution refinement not available, using original solutions"
            )

    # Build highlighted preview with full clause text and track which clauses were highlighted
    highlighted_preview, highlighted_indices, expanded_clause_texts = (
        _build_highlighted_preview(full_text, deduped_clauses)
    )
    ...
    for clause_idx, clause in enumerate(deduped_clauses):
        ...
        if expanded_text:
            ...
            logger.info(
                f"Using expanded clause text ({len(expanded_text)} chars): {expanded_text[:60]}..."
            )
        else:
            clause_text = clause.get("clause_text", "")
        ...
        if risk_score <= 1:
            logger.info(
                f"Filtering minimal risk clause (score {risk_score}): {clause_text[:60]}..."
            )
            continue
    ...
    logger.info(
        f"Input data: doc_type={doc_type}, doc_type_name={doc_type_name}, chunk_results={len(chunk_results)}, deduped_clauses={len(deduped_clauses)}"
    )
    ...
    if use_llm_for_summary:
        logger.info(
            "Attempting LLM-based comprehensive summary generation (will fallback to regex if quota exceeded)..."
        )
        try:
            ...
            if comprehensive_summary:
                logger.info(
                    f"✅ LLM-based comprehensive summary generated successfully"
                )
                logger.info(f"Summary keys: {list(comprehensive_summary.keys())}")
                logger.info(
                    f"Parties extracted: {len(comprehensive_summary.get('parties', []))}"
                )
                logger.info(
                    f"Financial terms: {len(comprehensive_summary.get('financial_terms', []))}"
                )
                logger.info(
                    f"Legal terms explained: {len(comprehensive_summary.get('legal_terms_explained', []))}"
                )
        except Exception as exc:
            error_msg = str(exc)
            is_quota = any(
                indicator in error_msg.lower()
                for indicator in ["quota", "rate limit", "429", "resource exhausted"]
            )
            if is_quota:
                logger.warning(
                    f"LLM quota exceeded, automatically falling back to regex extraction"
                )
            else:
    ...
    if not comprehensive_summary:
        ...
        comprehensive_summary = {
            ...
            "legal_terms_explained": [],
            "executive_summary": (
                summary_parts[0] if summary_parts else "Document analysis completed."
            ),
        }
    ...
    logger.info(
        f"✅ Response data prepared with comprehensive_summary: {comprehensive_summary is not None}"
    )
...
def chat_with_document(session, user_message):
    ...
    try:
        ...
        recent_messages = ChatMessage.objects(session=session).order_by("created_at")[
            :10
        ]
        ...
        for clause in (session.high_risk_clauses or [])[:5]:
            ...
            risk_score = _coerce_risk_score(
                clause.get("risk_score") or clause.get("riskScore"), default=3
            )
            risk_level = (
                clause.get("risk_level")
                or clause.get("riskLevel")
                or _score_to_label(risk_score)
            )
            mitigation = (
                (
                    clause.get("mitigation")
                    or clause.get("recommendation")
            ...
            replacement = (
                (
                    clause.get("replacement_clause")
                    or clause.get("replacementClause")
                    or clause.get("alternate_clause")
                    or ""
                )
                .replace("\n", " ")
                .strip()
        ...
        risk_overview = (
            "\n".join(risk_lines)
            if risk_lines
            else "No high risk clauses were highlighted in the initial analysis."
        )
        ...
        if preview_for_prompt:
            ...
            preview_for_prompt = preview_for_prompt.replace(
                '<mark class="high-risk">', "[HIGH-RISK]"
            ).replace("</mark>", "[/HIGH-RISK]")
        ...
        messages = [
            {
                "role": "user",
                "parts": [
                    f"""You are a legal assistant helping a user understand a legal document.
...
"""
            ...
            {
                "role": "model",
                "parts": ["Okay, I am ready to help you with your document."],
            },
        ]

        for msg in recent_messages:
            role = (
                "user" if msg.is_user else "model"
...
def summarize_document(request):
    ...
    try:
        ...
        session.highlighted_preview = analysis.get(
            "highlighted_preview"
        ...
        return Response(
            {
                ...
                "comprehensive_summary": analysis.get(
                    "comprehensive_summary"
                ),  # ADD THIS
                ...
                "document_type_confidence": analysis.get(
...
def chat_message(request):
    ...
    try:
        ...
        # Save user message
        user_msg = ChatMessage(session=session, message=user_message, is_user=True)
        user_msg.save()
        ...
        # Save AI response
        ai_msg = ChatMessage(session=session, message=ai_response, is_user=False)
        ai_msg.save()

        return Response(
            {"response": ai_response, "message_id": str(ai_msg.id)},
        ...
        return Response(
            {
                ...
                "session": {
                    ...
                    "document_type_confidence": session.document_type_confidence
                    or None,
...
def user_sessions(request):
    ...
    try:
        ...
        logger.info(
            f"Fetching sessions for user: {user.email if hasattr(user, 'email') else user}"
        )
        ...
        try:
            ...
            logger.info(
                f"Found {len(sessions)} sessions for user {user.email if hasattr(user, 'email') else user}"
            )
        ...
        try:
            ...
            message_counts = list(
                ChatMessage._collection.aggregate(
                    [
        ...
        for session in sessions:
            try:
                ...
                sessions_data.append(
                    {
                        ...
                        "document_preview": (
                            (
                                session.document_text[:100] + "..."
                                if len(session.document_text) > 100
                                else session.document_text
                            )
                ...
                logger.error(
                    f"Error processing session {session.id}: {str(session_error)}"
                )
...
def task_status(request, session_id):
    ...
    try:
        ...
        response_data = {"session_id": str(session.id), **status_info}
