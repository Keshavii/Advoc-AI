...
const LawyerProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await axios.get(`api/auth/lawyers/${id}/`);
  ...
  const handleConnect = async () => {
    ...
    const message = window.prompt("Share a short note for the lawyer (optional):", "");
    ...
    try {
      const response = await axios.post(`api/auth/lawyers/${profileData.user.id}/connect/`, {
        message: message || "",
      ...
      toast.success(
        meetingLink
          ? `Connection request submitted! Google Meet link: ${meetingLink}`
          : response.data?.message || "Connection request submitted!"
      );
    } catch (err) {
